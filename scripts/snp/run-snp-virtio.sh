...
sudo \
    ...
    -netdev tap,ifname=tap0,br=br1,id=net0,script=no,downscript=no,vhost=on,queues=${SMP} \
    -device virtio-net-pci,netdev=net0,rss=on,hash=on,mq=on,vectors=$(($SMP*2+2)),rx_queue_size=256,tx_queue_size=256,mrg_rxbuf=on,iommu_platform=off,csum=off,guest_csum=off,guest_tso4=off,guest_tso6=off
